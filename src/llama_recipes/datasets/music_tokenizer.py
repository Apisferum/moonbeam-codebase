...
class MusicTokenizer():
    ...
    def convert_to_language_tokens(self, x):
        ...
        # --- OOV SAFETY FIX: Cap out-of-bounds timeshifts and durations ---
        # Prevents crashes from massively long sustained notes (e.g., 5201 ticks)
        max_ts = max(self.timeshift_dict.keys())
        max_dur = max(self.duration_dict.keys())
        
        # If the value isn't in the dict (or exceeds the max), fallback to the max allowed
        safe_ts = x[1] if x[1] in self.timeshift_dict else max_ts
        safe_dur = x[2] if x[2] in self.duration_dict else max_dur
        
        out.append(self.timeshift_dict[safe_ts])
        out.append(self.duration_dict[safe_dur])
        # -----------------------------------------------------------------
