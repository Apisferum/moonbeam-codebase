...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        # --- PERFORMANCE FIX: CACHE TENSORS ON GPU TO ELIMINATE PCIe BOTTLENECK ---
        # We attach the cache to `self` so it survives across all batches and epochs.
        # It only builds ONCE on Step 1, and then reads instantly from GPU VRAM.
        ...
        position_ids_sos = torch.where(where_sos, self._pos_tensor_cache["sos"], position_ids)
        position_ids_eos = torch.where(where_eos, self._pos_tensor_cache["eos"], position_ids_sos)
        ...
        if additional_token_map is not None and additional_tokens_pos_map is not None:
            for token_id in additional_token_map:
                ...
                # Fetch instantly from GPU memory instead of CPU->PCIe->GPU transfer
                cached_tensor = self._pos_tensor_cache.get(str_id, self._pos_tensor_cache["default"])
                position_ids = torch.where(where_new_token, cached_tensor, position_ids)
    ...
    def embed_tokens(self, input_ids, additional_token_map = None):
        ...
        if additional_token_map is not None:
            ...
            for token_id, embed_idx in additional_token_map.items():
                # 🛠️ DEVICE SYNC FIX: Force dynamic embedding to input device
                if self.supplementary_embedding_metadata.weight.device != input_ids.device:
                    self.supplementary_embedding_metadata.to(input_ids.device)
...
class LlamaForCausalLM_Conditional_Generation(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # --- GOD-TIER OPTIMIZATION: PRE-CACHE CHORD MIDI PITCHES ---
        # Eliminates music21 from the forward pass entirely!
        self._chord_midi_cache = {}
        for symbol in self.chord_idx2symbol.values():
            if symbol != "s":
                # Call music21 ONCE per chord type and save the tensor
                self._chord_midi_cache[symbol] = torch.tensor(chord_to_midi(symbol))
        # -----------------------------------------------------------
    ...
    def forward(
        ...
        if labels is not None: #if label exists: during training/evaluation --> teacher forcing, return loss;
            # --- CRITICAL FIX: MOVE TO CPU ONCE FOR ALL LOOPS ---
            ...
            # --- SMART DYNAMIC EXTRACTION OF metadata_condition ---
            if metadata_condition is None:
                metadata_condition = []
                valid_meta_tokens = set(additional_token_map.keys()) if additional_token_map else set()
                

                
                for b in range(batch_size):
                    batch_meta_tokens = []
                    sos_indices = (input_ids_cpu[b, :, 0] == self.sos_token).nonzero(as_tuple=True)[0]
                    sos_idx = sos_indices[0].item() if len(sos_indices) > 0 else seq_len
                    # Scan all 6 dimensions for the metadata tokens
                    for t in range(sos_idx):
                        for dim in range(6):
                            val = input_ids_cpu[b, t, dim].item() # Now reads instantly from CPU RAM!
                            if val in valid_meta_tokens:
                                batch_meta_tokens.append(val)
                                break
                            # Catch tokens hiding in the 6th dimension as positive numbers (e.g., 351 -> -351)
                            elif dim == 5 and -val in valid_meta_tokens:
                                batch_meta_tokens.append(-val)
                                break
                                
                    # --- CRITICAL FIX: Pad/Truncate to exactly 11 tokens ---
                    # The author's gru_condition_layer strictly expects 11 * hidden_size (21120).
                    if len(batch_meta_tokens) == 0:
                        batch_meta_tokens = [-4] * 11 # Fallback to SOC token
                    elif len(batch_meta_tokens) < 11:
                        pad_token = batch_meta_tokens[0]
                        batch_meta_tokens.extend([pad_token] * (11 - len(batch_meta_tokens)))
                    elif len(batch_meta_tokens) > 11:
                        batch_meta_tokens = batch_meta_tokens[:11]
                    # -------------------------------------------------------
                    
                    metadata_condition.append(batch_meta_tokens)
            # -------------------------------------------------------
            logits_list = []
            labels_list = []
            ...
            for batch_idx in range(batch_size):
                # --- USE CPU TENSOR TO PREVENT GPU STALLS ---
                sos_indices = (input_ids_cpu[batch_idx, :, 0] == self.sos_token).nonzero(as_tuple=True)[0]
                seq_indices_sos = sos_indices[0].item() if len(sos_indices) > 0 else seq_len
                
                eos_indices = (input_ids_cpu[batch_idx, :, 0] == self.eos_token).nonzero(as_tuple=True)[0]
                seq_indices_eos = eos_indices[0].item() if len(eos_indices) > 0 else seq_len
                # ---------------------------------------------
                ...
                if self.if_add_metadata_in_decoder:
                    # Fallback: If extraction somehow failed, use a dummy zero tensor to prevent shape crashes
                    if len(metadata_condition[batch_idx]) == 0:
                        expected_meta_size = self.gru_condition_layer.weight.shape[1]
                        dummy_meta = torch.zeros(expected_meta_size, device=input_ids.device, dtype=shift_logits_x.dtype)
                        metadata_condition_embedded_single = dummy_meta.unsqueeze(0).expand(seq_indices_eos - seq_indices_sos, -1)
                    else:
                        metadata_condition_embedded_single = self.model.supplementary_embedding_metadata(torch.tensor([additional_token_map[int(token)] for token in metadata_condition[batch_idx]]).to(input_ids)).reshape(-1).unsqueeze(0).expand(seq_indices_eos - seq_indices_sos, -1).to(shift_logits_x)
            ...
            if self.decoder_attn_implementation == "GRU":    
                ...
                if self.if_add_metadata_in_decoder:
                    # 🛠️ DEVICE SYNC FIX: Force dynamic embedding to input device
                    target_device = decoded_language_tokens.device
                    if self.model.supplementary_embedding_metadata.weight.device != target_device:
                        self.model.supplementary_embedding_metadata.to(target_device)

                    metadata_condition = self.model.supplementary_embedding_metadata(
                        torch.tensor([
                            additional_token_map[token.item()] 
                            for batch in metadata_condition 
                            for token in batch                  
                        ], device=target_device)
                    ).reshape(metadata_condition.shape[0] , -1).unsqueeze(1) 
