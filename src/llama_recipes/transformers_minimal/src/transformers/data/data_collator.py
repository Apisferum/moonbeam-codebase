...
def torch_default_data_collator(features):
    print("🚀 MOONBEAM 6D COLLATOR ACTIVATED!", flush=True)
    import torch
    ...
    def to_tensor(x):
        if not isinstance(x, torch.Tensor):
            x = torch.tensor(x)
        # Only strip a genuine leading batch dim of size 1 (e.g. (1, seq_len, 6)),
        # never collapse the sequence dim itself.
        if x.dim() == 3 and x.shape[0] == 1:
            x = x.squeeze(0)
        return x

    # max_len from input_ids (source of truth for sequence length)
    max_len = 0
    for f in features:
        if "input_ids" in f:
            t = to_tensor(f["input_ids"])
            max_len = max(max_len, t.shape[0])

    def pad_to(t, pad_len, pad_val):
        if pad_len == 0:
            return t
        if pad_len < 0:
            raise ValueError(
                f"Sequence length {t.shape[0]} exceeds computed max_len "
                f"{t.shape[0] + pad_len}; got a longer field than input_ids."
            )
    ...
    for k in ["input_ids", "labels", "attention_mask"]:
        if not any(k in f or (k == "attention_mask" and "input_ids" in f) for f in features):
            continue
        ...
        for f in features:
            if k in f:
                ...
                t = pad_to(t, max_len - t.shape[0], pad_val)
                padded_list.append(t)
            elif k == "attention_mask" and "input_ids" in f:
                t_ids = to_tensor(f["input_ids"])
                pad_len = max_len - t_ids.shape[0]
                mask = torch.cat([
                    torch.ones(t_ids.shape[0], dtype=torch.long, device=t_ids.device),
                    torch.zeros(pad_len, dtype=torch.long, device=t_ids.device),
                ])
                padded_list.append(mask)
            else:
                # Key missing for this sample and can't be derived: fail loudly
                # instead of silently shifting every subsequent example.
                raise KeyError(
                    f"Sample missing required field '{k}' and it cannot be derived; "
                    f"refusing to silently misalign the batch."
                )

        batch[k] = torch.stack(padded_list)
